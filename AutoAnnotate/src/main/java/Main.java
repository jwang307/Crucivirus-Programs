...
public class Main {
    ...
    public static void main(String[] args) throws  IOException {
        if (args.length < 3)
            throw new IllegalArgumentException("Specify 3 parameters: gff input file name, fasta input file name, and gff output file name");

        FeatureList gffInFeatures = GFF3Reader.read(args[0]);
        List<String> sense = getSenses(args[0]);

        String cmd = "hmmsearch repprofile.hmm " + args[1];
        ...
        try{
            ...
            hmmRun.waitFor();
            System.out.println(hmmRun.exitValue());
        } catch (Exception e){
            e.printStackTrace();
            System.out.println(e.getMessage());
        }

        List<HMMHit> hits = readHMMER();
    ...
    public static List<HMMHit> readHMMER() throws IOException {
        BufferedReader hmmerIn = new BufferedReader(new InputStreamReader(System.in));
